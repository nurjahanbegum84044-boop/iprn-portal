...
async function fetchMessages(ctx, targetNumber) {
    ...
    try {
        const apiUrl = `${BASE_URL}?token=${API_TOKEN}&limit=20`;

        const response = await axios.get(apiUrl, { timeout: 8000 });
        const resData = response.data;
        
        // এপিআই রেসপন্সের 'records' থেকে মেসেজগুলো নেওয়া হচ্ছে
        ...
        // যদি নাম্বার দিয়ে সার্চ করা হয়, তবে ওই নাম্বারের মেসেজ ফিল্টার করবে
        ...
        if (filteredMessages.length === 0) {
            return ctx.reply(`❌ \`${targetNumber}\` এই নম্বরের জন্য কোনো ওটিপি পাওয়া যায়নি।`);
        }
